...
def bbox_ious(atlbrs, btlbrs):
    """
    Compute IoU between two sets of bounding boxes using pure NumPy.
    Pure Python/NumPy implementation to avoid cython_bbox dependency.
    
    :param atlbrs: np.ndarray of shape (N, 4) in tlbr format (x1, y1, x2, y2)
    :param btlbrs: np.ndarray of shape (M, 4) in tlbr format (x1, y1, x2, y2)
    :return: IoU matrix of shape (N, M)
    """
    atlbrs = np.atleast_2d(atlbrs)
    btlbrs = np.atleast_2d(btlbrs)
...
def linear_assignment(cost_matrix, thresh):
    ...
    matched_rows = set(matches[:, 0].tolist()) if len(matches) > 0 else set()
    matched_cols = set(matches[:, 1].tolist()) if len(matches) > 0 else set()

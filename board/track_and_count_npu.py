...
# 健康预警模块（体重估计 + 健康评分）
from health_module import diagnose_trajectory, aggregate_herd, set_weight_model
...
class ZoneAnalyzer:
    ...
    def update(self, track_id, cx, frame_idx, cy=None, w=None, h=None, conf=None, roi=None):
        ...
        if track_id not in self.trajectories:
            ...
            self.trajectories[track_id].add_point(zone, frame_idx, self.fps, cx, cy, w, h, conf, roi=roi)
            if old_zone != zone:
                self.id_events.append({
                    'frame': frame_idx, 'timestamp': f"{frame_idx / self.fps:.2f}s",
...
def main():
    ...
    parser.add_argument('--weight_om', type=str, default='',
                        help='Path to weight regressor .om. DISABLED by default (heuristic only). '
    ...
    # Weight regressor is DISABLED by default (heuristic only).
    # To re-enable, pass an explicit path: --weight_om /path/to/weight_regressor_fp16.om
    # The .om file and weight_om.py are preserved on disk for future opt-in use.
    if args.weight_om and Path(args.weight_om).exists():
        try:
        ...
        print("[health] weight model disabled (default); using heuristic")

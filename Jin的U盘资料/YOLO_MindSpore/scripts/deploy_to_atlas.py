...
DEPLOY_LOCAL = PROJECT_ROOT / "app" / "atlas_runtime"
TRACKING_LOCAL = PROJECT_ROOT / "app" / "tracking" / "byte_tracker"
AGENT_LOCAL = PROJECT_ROOT / "app" / "agents" / "pig_counting_agent.py"
BOOTSTRAP_LOCAL = PROJECT_ROOT / "deploy" / "atlas" / "bootstrap_board.sh"
LOCAL_MODEL = PROJECT_ROOT / "yolov8n_pig_fp16.om"
DATASET_DIRS = {
    "4": PROJECT_ROOT / "data" / "datasets" / "group4",
    "5": PROJECT_ROOT / "data" / "datasets" / "group5",
}
REMOTE_DATASETS = {
    "4": "group4",
    "5": "group5",
...
def main() -> None:
    ...
    if not TRACKING_LOCAL.exists():
        raise FileNotFoundError(f"Tracking bundle missing: {TRACKING_LOCAL}")
    if not AGENT_LOCAL.exists():
        raise FileNotFoundError(f"Agent module missing: {AGENT_LOCAL}")
    if not BOOTSTRAP_LOCAL.exists():
        raise FileNotFoundError(f"Bootstrap script missing: {BOOTSTRAP_LOCAL}")
    ...
    try:
        ...
        sftp_upload_dir(
            sftp,
            TRACKING_LOCAL,
            f"{args.remote_dir}/trackers/byte_tracker",
            skip_names={"__pycache__"},
        )
        sftp_upload_file(sftp, AGENT_LOCAL, f"{args.remote_dir}/pig_counting_agent.py")
        sftp_upload_file(sftp, BOOTSTRAP_LOCAL, f"{args.remote_dir}/bootstrap_board.sh")

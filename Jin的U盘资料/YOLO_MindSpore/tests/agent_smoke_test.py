...
ATLAS_RUNTIME = ROOT / "app" / "atlas_runtime"
PROJECT_ROOT = ROOT
for path in [ROOT, PROJECT_ROOT, ATLAS_RUNTIME]:
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from legacy.review_agent import HumanReviewAgent
from legacy.diagnosis_agent import DiagnosisAgent
from app.atlas_runtime.autonomous_agent import AutonomousOpsAgent


def _import_web_monitor():
    """Import web_monitor with mocked hardware dependencies."""
    for mod_name in ['cv2', 'numpy', 'npu_detector',
                     'app.tracking', 'app.tracking.byte_tracker',
                     'app.tracking.byte_tracker.byte_tracker',
                     'track_and_count_npu']:
    ...
    bt_mod = sys.modules['app.tracking.byte_tracker.byte_tracker']
    class FakeBYTE:
        def __init__(self, *a, **k): pass
        def update(self, *a, **k): return []
    bt_mod.BYTETracker = FakeBYTE

    import app.atlas_runtime.web_monitor as web_monitor
    return web_monitor
...
def test_generated_review_csv():
    csv_path = ROOT / "output" / "batch_rerun_group4_mindspore_review" / "batch_rerun_group4_results.csv"
    if not csv_path.exists():
        csv_path = ROOT / "output" / "batch_rerun_group4_review" / "batch_rerun_group4_results.csv"
    if not csv_path.exists():
...
def test_diagnosis_agent():
    out_dir = ROOT / "output" / "batch_rerun_group4_mindspore_review" / "17-37头"
    if not out_dir.exists():
        out_dir = ROOT / "output" / "batch_rerun_group4_review" / "17-37头"
    if not out_dir.exists():
